...
# --- 設定與路徑 ---
# ⚠️ 請確保 'datas/raw/8_BFI82U' 路徑存在或可被建立
...
def fetch_twse_bfi82u_single(target_date: str) -> Optional[pd.DataFrame]:
    """
    抓取指定日期的 BFI82U 報告 (三大法人買賣超金額)。
    ...
    # 注意 BFI82U 的 URL 參數是 dayDate，且需要 type=day
    url = f"{BASE_URL}?type=day&dayDate={target_date}&response=csv"
    
    # 使用 os.path.join 確保跨平台兼容性
    filename = os.path.join(OUTPUT_DIR, f"{target_date}_BFI82U_3IParty_Day.csv") 
...
def batch_fetch_twse_bfi82u(date_list: List[str]):
    """
    針對提供的日期清單，逐一抓取 TWSE BFI82U 資料，並在失敗時實作重試機制。
    """
    print("--- 開始批次抓取 TWSE BFI82U 資料 ---")
    ...
    for target_date in date_list:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            # 執行抓取
            df = fetch_twse_bfi82u_single(target_date)
...
if __name__ == "__main__":
    ...
    if final_date_list:
        print("--- 開始抓取/檢查檔案 ---")
        batch_fetch_twse_bfi82u(final_date_list)
    else:
