...
# --- 設定與路徑 ---
# ⚠️ 請確保 'datas/raw/4_MI_INDEX20' 路徑存在或可被建立
...
# 依據現在時間是 2025 年 11 月，我們應該使用 trading_day_2021-2025.csv
...
def _fetch_twse_data(url: str) -> Optional[str]:
    ...
    try:
        ...
        # 檢查 TWSE 回傳內容是否為錯誤訊息
        # MI_INDEX20 的錯誤訊息可能是 "很抱歉..." 或 "查無相關資料"
...
def _read_twse_csv(response_text: str, header_row: int) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if not df.empty:
            ...
            # 額外清理：MI_INDEX20 (市場別) 的資料可能以「市場別」作為第一欄位，移除空白行
            if '市場別' in df.columns:
                 df = df[df['市場別'].astype(str).str.strip() != '']
...
def fetch_twse_mi_index20_single(target_date: str) -> Optional[pd.DataFrame]:
    """
    抓取指定日期的 MI_INDEX20 報告。
    ...
    # 根據您的要求設定 URL 和 filename
    url = f"{BASE_URL}?date={target_date}&response=csv"
    # 使用 os.path.join 確保跨平台兼容性
    filename = os.path.join(OUTPUT_DIR, f"{target_date}_MI_INDEX20_Market.csv") 
    ...
    # MI_INDEX20 報表頭部可能也有兩行註解，使用 header_row=1 嘗試
    df = _read_twse_csv(response_text, header_row=1) 

    if df is not None:
        # 由於 MI_INDEX20 格式相對簡單，這裡不做額外清理，_read_twse_csv已處理基礎清理
...
def batch_fetch_twse_mi_index20(date_list: List[str]):
    """
    針對提供的日期清單，逐一抓取 TWSE MI_INDEX20 資料，並在失敗時實作重試機制。
    """
    print("--- 開始批次抓取 TWSE MI_INDEX20 資料 ---")
    ...
    for target_date in date_list:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            # 執行抓取
            df = fetch_twse_mi_index20_single(target_date)
...
if __name__ == "__main__":
    ...
    if final_date_list:
        print("--- 開始抓取/檢查檔案 ---")
        batch_fetch_twse_mi_index20(final_date_list)
    else:
