...
# --- 設定與路徑 ---
# ⚠️ 請確保 'datas/raw/6_BFIAMU' 路徑存在或可被建立
...
def _read_twse_csv(response_text: str, header_row: int) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if not df.empty:
            ...
            # BFIAMU 報表通常以「自營商」作為第一欄位，移除空白行
            if '自營商' in df.columns:
                 df = df[df['自營商'].astype(str).str.strip() != '']
...
def fetch_twse_bfiamu_single(target_date: str) -> Optional[pd.DataFrame]:
    """
    抓取指定日期的 BFIAMU 報告 (自營商買賣金額)。
    ...
    filename = os.path.join(OUTPUT_DIR, f"{target_date}_BFIAMU_DealerTrade.csv") 
...
def batch_fetch_twse_bfiamu(date_list: List[str]):
    """
    針對提供的日期清單，逐一抓取 TWSE BFIAMU 資料，並在失敗時實作重試機制。
    """
    print("--- 開始批次抓取 TWSE BFIAMU 資料 ---")
    ...
    for target_date in date_list:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            # 執行抓取
            df = fetch_twse_bfiamu_single(target_date)
...
if __name__ == "__main__":
    ...
    if final_date_list:
        print("--- 開始抓取/檢查檔案 ---")
        batch_fetch_twse_bfiamu(final_date_list)
    else:
