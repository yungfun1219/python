...
def calculate_kdj(df, n=9, m1=3, m2=3, high_col='最高價', low_col='最低價', close_col='收盤價'):
    ...
    K = (K-1) * (M1-1)/M1 + RSV * 1/M1 (M1 日平滑移動平均)
    D = (D-1) * (M2-1)/M2 + K * 1/M2 (M2 日平滑移動平均)
    J = 3 * K - 2 * D
    """
    ...
    df['RSV'] = (df[close_col] - df['LLV']) / denominator * 100
    df['RSV'] = df['RSV'].fillna(0).clip(0, 100) # 將 NaN 設為 0，並限制在 0-100 之間

    # 3. 計算 K 線 (M1 日 EMA/SMA，這裡使用 SMA 邏輯，但使用 .ewm 實現平滑移動平均)
    # 初始值通常設為 50 或 RSV 的第一個非空值
    
    # 確保 K、D 的初始值為 50.0 (如果資料不足 N 日)
    initial_k = 50.0
    initial_d = 50.0

    # 由於 Pandas .ewm 無法直接設置初始值，我們使用 for 迴圈或替代的平滑方法
    df['K'] = 0.0
    df['D'] = 0.0

    # 採用簡單的 for 迴圈來實現平滑移動平均，這是最符合 KDJ 經典定義的做法
    k_list = []
    d_list = []
    
    # 根據定義，如果沒有前一個值，通常設定初始值為 50
    k_prev = initial_k
    d_prev = initial_d
    
    # KDJ 計算通常從第一個有 RSV 值的位置開始
...
def output_to_csv(df, output_path, raw_cols):
    """
    🎯 修改: 將 DataFrame 輸出為 CSV 檔案 (big5 編碼)。
    確保輸出所有原始欄位 + 技術指標 (包含 KDJ)。
    """
    # 將日期格式化為字串
    ...
    # 🌟 增加 K, D, J 欄位
    tech_cols = ['MA5', 'MA10', 'MA20', 'DIF', 'DEA', 'OSC', 'K', 'D', 'J']
    ...
    try:
        ...
        print(f"✅ 已輸出 CSV（big5，包含所有原始欄位/MA/MACD/KDJ）到：{output_path}")
        return True
    except Exception as e:
        print(f"❌ 輸出 CSV 失敗: {e}")
        return False
