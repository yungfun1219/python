...
# --- 設定與路徑 ---
# ⚠️ 請確保 'datas/raw/11_T86' 路徑存在或可被建立
...
def get_date_list_based_on_time(file_path: str) -> Optional[List[str]]:
    ...
    # 目前時間是 9:23:52 CST，早於 21:00，截止日為昨天 (11/22)
...
def _read_twse_csv(response_text: str, header_row: int) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if not df.empty:
            ...
            # T86 報表通常以「證券代號」作為第一欄位，移除空白行
            # 注意: T86 的 CSV 有時會包含結尾的匯總資訊，需要額外清理 (這裡先假設 CSV 只有單表)
            if '證券代號' in df.columns:
                 # 移除「證券代號」為空的行，通常是資料尾部的總計行或空行
                 df = df[df['證券代號'].astype(str).str.strip() != '']
                 
            # 重新檢查，確保 dataframe 非空
            if df.empty:
                print("⚠️ 解析 CSV 後 DataFrame 為空，可能無有效資料。")
                return None
...
def fetch_twse_t86_single(target_date: str) -> Optional[pd.DataFrame]:
    """
    抓取指定日期的 T86 報告 (三大法人買賣超統計 - ALL)。
    ...
    # 注意 T86 的 URL 參數包含 selectType=ALL
...
def batch_fetch_twse_t86(date_list: List[str]):
    """
    針對提供的日期清單，逐一抓取 TWSE T86 資料，並在失敗時實作重試機制。
    """
    print("--- 開始批次抓取 TWSE T86 資料 ---")
    ...
    for target_date in date_list:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            # 執行抓取
            df = fetch_twse_t86_single(target_date)
...
if __name__ == "__main__":
    ...
    if final_date_list:
        print("--- 開始抓取/檢查檔案 ---")
        batch_fetch_twse_t86(final_date_list)
    else:
