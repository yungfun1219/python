...
def main():
    ...
    data_8039 = load_and_process_data(STOCK_LIST[2]['code'], STOCK_LIST[2]['name'], OUTPUT_DIR)
    data_8110 = load_and_process_data(STOCK_LIST[3]['code'], STOCK_LIST[3]['name'], OUTPUT_DIR)
    
    if data_2344 is None or data_1802 is None or data_8039 is None or data_8110 is None:
        print("\n❌ 由於資料缺失，無法繪製合併圖表。請檢查資料檔案是否存在。")
        return
        
    print("\n========================================================")
    print("📈 開始繪製 2x2 網格 K 線圖 (定位於螢幕上半部)")
    print("========================================================")
    ...
    # 6. 創建總體 Figure (畫布) 和 2x2 的子圖 Axes (畫板)
    # 🌟 核心修正 1: 變更為 2x2，並調整 figsize 保持比例
    fig, axes = plt.subplots(2, 2, figsize=(20, 12)) 
    axes = axes.flatten() # 將 2x2 陣列攤平，方便用單一索引 [0], [1], [2], [3]

    # 調整邊界以最大化圖表區域
    # 🌟 核心修正 2: 增加垂直間距 hspace
    fig.subplots_adjust(left=0.03, right=0.97, top=0.95, bottom=0.08, hspace=0.35, wspace=0.1)
    ...
    # 7. 繪製第一張圖 (2344 華邦電) - axes[0]
    stock_info = STOCK_LIST[0]
    mpf.plot(data_2344, ax=axes[0], type='candle', mav=mav_periods, mavcolors=mavcolors, volume=False, style=style, ylabel='價格 (TWD)')
    axes[0].set_title(f"{stock_info['code']} ({stock_info['name']}) 近 90 天 K 線 ({data_2344.index.min().date()} ~ {data_2344.index.max().date()})", fontsize=16)
    axes[0].legend(handles=legend_elements, loc='upper left', fontsize=12)
    
    # 8. 繪製第二張圖 (1802 台玻) - axes[1]
    stock_info = STOCK_LIST[1]
    mpf.plot(data_1802, ax=axes[1], type='candle', mav=mav_periods, mavcolors=mavcolors, volume=False, style=style, ylabel='價格 (TWD)')
    axes[1].set_title(f"{stock_info['code']} ({stock_info['name']}) 近 90 天 K 線 ({data_1802.index.min().date()} ~ {data_1802.index.max().date()})", fontsize=16)
    axes[1].legend(handles=legend_elements, loc='upper left', fontsize=12)

    # 9. 繪製第三張圖 (8039 台虹) - axes[2]
    stock_info = STOCK_LIST[2]
    mpf.plot(data_8039, ax=axes[2], type='candle', mav=mav_periods, mavcolors=mavcolors, volume=False, style=style, ylabel='價格 (TWD)')
    axes[2].set_title(f"{stock_info['code']} ({stock_info['name']}) 近 90 天 K 線 ({data_8039.index.min().date()} ~ {data_8039.index.max().date()})", fontsize=16)
    axes[2].legend(handles=legend_elements, loc='upper left', fontsize=12)

    # 10. 繪製第四張圖 (8110 華東) - axes[3]
    stock_info = STOCK_LIST[3]
    mpf.plot(data_8110, ax=axes[3], type='candle', mav=mav_periods, mavcolors=mavcolors, volume=False, style=style, ylabel='價格 (TWD)')
    axes[3].set_title(f"{stock_info['code']} ({stock_info['name']}) 近 90 天 K 線 ({data_8110.index.min().date()} ~ {data_8110.index.max().date()})", fontsize=16)
    axes[3].legend(handles=legend_elements, loc='upper left', fontsize=12)

    
    # 11. 儲存合併後的圖表
    chart_filename = f"Combined_KLine_90Days_4_Stocks.png"
    save_chart_path = OUTPUT_DIR / chart_filename
    try:
        fig.savefig(save_chart_path)
        print(f"📈 4 檔股票合併圖表已儲存到：{save_chart_path}")
    except Exception as e:
        print(f"❌ 合併圖表儲存失敗: {e}")

    # 12. 顯示圖表 (嘗試定位到螢幕上半部)
    print("\n👀 正在顯示 2x2 網格 K 線圖 (定位於螢幕上半部)...")
    try:
        # 設置圖表大小和位置
        fig_manager = plt.get_current_fig_manager()

        # 調整目標尺寸以容納 4 個圖表，並保持在螢幕上半部 (約 1600x900 像素)
        target_width = 1600
        target_height = 900 
