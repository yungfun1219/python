...
# -----------------------------------------------------------
# 【🌟 最終修正後的繪圖函式：解決 KeyError 並調整圖例位置】
# -----------------------------------------------------------
def plot_charts_four_panels(df, code, name, output_dir):
    """
    繪製 K 線圖、MACD、KDJ、RSI 的 4x1 圖表。
    - 解決 mplfinance 版本問題 (移除 show_titles)。
    - K 線圖手動添加 MA 圖例，並將所有圖例移至左上方。
    - 日期水平顯示。
    """
    if df.empty:
        print("❌ 繪圖失敗：數據不足。")
    ...
    # 確保技術指標計算所需數據存在，否則繪圖會報錯
    df_tech = df_ohlc.dropna(subset=['DIF', 'DEA', 'OSC', 'K', 'D', 'J', 'RSI5', 'RSI10']).copy()
    ...
    # --- 創建 Figure 和 Axes (4x1 佈局) ---
    fig, axes = plt.subplots(4, 1, figsize=(16, 18), 
                             gridspec_kw={'height_ratios': [3, 1, 1, 1]}) 
    
    # 調整子圖間距
    fig.subplots_adjust(left=0.06, right=0.97, top=0.94, bottom=0.05, hspace=0.3)
    
    ax_kline = axes[0] # K 線圖
    ax_macd = axes[1]  # MACD 圖
    ax_kdj = axes[2]   # KDJ 圖
    ax_rsi = axes[3]   # RSI 圖 (保留日期)
    ...
    mpf.plot(
        ...
        # 移除 show_titles=True，避免 KeyError
    )
    
    # 🌟 修正 1 & 2: 手動創建並添加 MA 圖例，定位於左上方
    legend_handles = []
    ...
    ax_kdj.axhline(80, color='red', linestyle=':', linewidth=1) # 超買線
    ax_kdj.axhline(20, color='green', linestyle=':', linewidth=1) # 超賣線
    ax_kdj.set_ylim(0, 100) # 限制 KDJ 範圍
    # 🌟 修正 2: 圖例顯示在左上方
    ...
    ax_rsi.axhline(70, color='red', linestyle=':', linewidth=1) # 超買區
    ax_rsi.axhline(30, color='green', linestyle=':', linewidth=1) # 超賣區
    ax_rsi.set_ylim(0, 100) # 限制 RSI 範圍
    # 🌟 修正 2: 圖例顯示在左上方
    ax_rsi.legend(loc='upper left', fontsize=10)
    ax_rsi.grid(True, linestyle='--', alpha=0.6)
    ax_rsi.set_xlabel('日期', fontsize=12) 

    # --- 統一處理 X 軸：隱藏前三張圖的 X 軸刻度和標籤 ---
    for i in range(len(axes) - 1): # 遍歷前 3 張圖 (索引 0, 1, 2)
        axes[i].tick_params(axis='x', labelbottom=False) 
        
    # 🌟 修正 3 (確認): 設置最後一張圖的 X 軸格式 (水平顯示，即 rotation=0)
    ax_rsi.tick_params(axis='x', rotation=0) 
    ax_rsi.xaxis.set_major_locator(plt.MaxNLocator(10))

    # --- 儲存與顯示 ---
    chart_filename = f"{code}_{name}_KLine_4Panel_90Days.png"
    save_path = output_dir / chart_filename
    try:
        fig.savefig(save_path)
        print(f"📈 整合圖表 (4 圖面板，日期水平顯示) 已儲存到：{save_path}")
    except Exception as e:
        print(f"❌ 整合圖表儲存失敗: {e}")
...
def main():
    ...
    # **注意：所有指標計算都必須在全量數據上執行，以確保 EMA/MA/KDJ 的準確性**
    ...
    # 6. 繪圖：整合 K 線、MACD、KDJ、RSI 指標圖表 (使用 90 天資料)
