...
if df_plot.empty:
    ...
    # 標題字串現在用於日誌訊息，而不是圖表標題
    chart_info_str = f'{STOCK_CODE} ({STOCK_NAME}) 近 90 天 K 線圖與移動平均線 (紅漲綠跌) ({df_plot["日期"].min().strftime("%Y/%m/%d")} ~ {latest_date.strftime("%Y/%m/%d")})'
    ...
    fig, axes = mpf.plot(
        ...
        volume=False,
        style=s, 
        title='',             # 【修正點】: 將 title 設置為空字串，不顯示
        ylabel='價格 (TWD)',   
        figscale=1.5,
        returnfig=True,
    )

    # 處理中文亂碼問題，確保 Y 軸、圖例等中文元素正確顯示
    if fig and axes:
    ...
    print(f"📈 合併圖表 (K線+MA, 紅漲綠跌，近 90 天) 已儲存至：{os.path.dirname(output_path)}/{combined_chart_filename} (無標題)") 
    # 這裡的 print 訊息可以保留，用於提示
