...
# --- 設定與路徑 ---
# ⚠️ 請確保 'datas/raw/5_TWTASU' 路徑存在或可被建立
...
def get_date_list_based_on_time(file_path: str) -> Optional[List[str]]:
    ...
    # 目前時間是 20:48:38 CST，早於 21:00，截止日為昨天 (11/21)
...
def _read_twse_csv(response_text: str, header_row: int) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if not df.empty:
            ...
            # TWTASU 報表通常以「市場別」或「項目」等作為第一欄位，移除空白行
            if '項目' in df.columns:
...
def fetch_twse_twtasu_single(target_date: str) -> Optional[pd.DataFrame]:
    """
    抓取指定日期的 TWTASU 報告。
    ...
    filename = os.path.join(OUTPUT_DIR, f"{target_date}_TWTASU_VolumePrice.csv") 
...
def batch_fetch_twse_twtasu(date_list: List[str]):
    """
    針對提供的日期清單，逐一抓取 TWSE TWTASU 資料，並在失敗時實作重試機制。
    """
    print("--- 開始批次抓取 TWSE TWTASU 資料 ---")
    ...
    for target_date in date_list:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            # 執行抓取
            df = fetch_twse_twtasu_single(target_date)
...
if __name__ == "__main__":
    ...
    if final_date_list:
        print("--- 開始抓取/檢查檔案 ---")
        batch_fetch_twse_twtasu(final_date_list)
    else:
