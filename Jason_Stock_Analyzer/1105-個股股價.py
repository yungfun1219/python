...
# 使用 relativedelta 計算往前推六個月的日期
target_date = current_date - relativedelta(months=6)
MIN_START_DATE_STR = target_date.strftime('%Y/%m/%d')
...
if __name__ == '__main__':
    ...
    count_stocks = len(all_codes)
    processed_count = 0

    print(f"\n--- 準備開始批量處理 {count_stocks} 檔股票 ---")

    # 2. 遍歷清單，逐一處理每檔股票
    for code in all_codes:
        processed_count += 1
        print(f"\n***** 處理進度: {processed_count}/{count_stocks} (代號: {code}) *****")
