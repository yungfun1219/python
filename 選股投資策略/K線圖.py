...
try:
    if os.path.exists(font_path):
        ...
        # print(f"【字型設定】成功載入中文字型：Microsoft JhengHei。")
    else:
...
def plot_candlestick_chart(df, title, months, filename):
    """
    使用 mplfinance 繪製指定期間的 K 線圖。
    
    Args:
        df (pd.DataFrame): 已經過處理的股票 OHLCV 資料。
        title (str): 圖表標題。
        months (int): 繪圖期間的月數 (例如 2 或 6)。
        filename (str): 儲存圖片的檔案名稱。
    """
...
if __name__ == '__main__':
    ...
    if data_df is not None:
        ...
        plot_candlestick_chart(
            ...
            months=2,
            filename='1101_candlestick_2months.png'
        )
        ...
        plot_candlestick_chart(
            ...
            months=6,
            filename='1101_candlestick_6months.png'
        )